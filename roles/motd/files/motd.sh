...
( : '~'.~.'~' : )  Load Averages......: ${one}, ${five}, ${fifteen} (1, 5, 15 min)
 ~ .~ (   ) ~. ~   Running Processes..: $(/bin/ps ax | /usr/bin/wc -l | /usr/bin/tr -d " ")
  (  : '~' :  )    IP Addresses.......: $(/sbin/ifconfig eth0 | /bin/grep "inet addr" | /usr/bin/cut -d ":" -f 2 | /usr/bin/cut -d " " -f 1) and $(/usr/bin/curl -s --connect-timeout 1 http://ipinfo.io/ip | /usr/bin/tail)
